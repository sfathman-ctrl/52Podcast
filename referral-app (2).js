function createProductCard(item) {
  ...
  if (hasContact) {
    ...
    if (item.contactName) {
      ...
      line.textContent = item.contactRole ? `${item.contactName} — ${item.contactRole}` : item.contactName;
...
function renderProducts() {
  ...
  if (!window.PRODUCTS || window.PRODUCTS.length === 0) {
    grid.innerHTML = '<p class="empty-state">No products yet — add some in referral-data.js.</p>';
    return;
  }
